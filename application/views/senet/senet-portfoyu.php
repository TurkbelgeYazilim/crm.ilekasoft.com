						...
						<div class="row">
							<div class="col-sm-10">
								...
								<ul class="breadcrumb">
									...
									<li class="breadcrumb-item">Senet
									</li>
									<li class="breadcrumb-item active">Senet Portföyü</li>
								</ul>
							</div>
						...
						<div class="col-xl-12 col-md-12">
							<div class="card card-table">
								<div class="card-header">
									<div class="row">
										<div class="col">
											<h5 class="card-title">Senet Portföyü</h5>
										</div>
										<div class="col-auto">
											<a href="<?= base_url("senet/yeni-senet"); ?>" class="btn btn-outline-success btn-sm"><i class="fa fa-plus"></i> Yeni Senet Giriş/Çıkış Ekle</a>
										</div>
										<!-- <div class="col-auto">
								...
								<div class="card-body">
									<div class="table-responsive">
										<table class="table table-striped custom-table mb-0">
											...
											<tbody>
											<?php foreach($senet as $senetler){?>

													<?php
														$hareketTipi = $senetler->senet_hareketTipi;
														$durum = $senetler->senet_durum;

														$borcluID = $senetler->senet_borcluID;
														$senetCariID = $senetler->senet_cariID;

														if($hareketTipi == 1){
															$hareketTipiText = "Alınan Senet";
															$cariQ = "SELECT * FROM cari WHERE cari_id = '$borcluID' ";
															$cari = $this->db->query($cariQ)->row();
															$firmaAd = $cari->cari_ad;
															//seçilen cari
														}else if($hareketTipi == 2){
															$hareketTipiText = "Verilen Senet";
															//kendi firmasının adı
															$cariQ = "SELECT * FROM cari WHERE cari_id = '$senetCariID' ";
															$cari = $this->db->query($cariQ)->row();
													...
													?>
												<tr>
													<td><?= $senetler->senet_portfoyNo; ?></td>
													<td><?= date('d.m.Y', strtotime($senetler->senet_kayitTarihi)); ?></td>
													<td><?= date('d.m.Y', strtotime($senetler->senet_vadeTarih)); ?></td>
													<td><?= $hareketTipiText; ?></td>
													<td><?= $firmaAd; ?></td>
													<td><?= $senetler->senet_tutar; ?></td>
													<td><?= $senetler->senet_notAciklama; ?></td>
													<td><?= $durumText; ?></td>
													<td class="text-right">
														<?php if($durum == 0){ ?>
															<a href="javascript:void(0);" data-toggle="modal" data-target="#edit_category" data-id="<?= $senetler->senet_id; ?>" data-tutar="<?= $senetler->senet_tutar; ?>"  class="btn btn-sm btn-white text-success mr-2"><i class="fa fa-money-bill mr-1"></i> Öde</a>
															<a href="<?= base_url("senet/senetduzenle/".$senetler->senet_id) ?>" class="btn btn-sm btn-white text-success mr-2"> Düzenle</a>
															<?php if ($deletePermission == 1) { ?>
																<a href="javascript:void(0);" data-toggle="modal" data-target="#sil" onclick="document.getElementById('senet_id').value='<?= $senetler->senet_id; ?>';document.getElementById('senet_no').innerHTML='Senet portföy numarası <?= $senetler->senet_portfoyNo; ?> olan seneti silmek istiyor musunuz?';"   class="btn btn-sm btn-white text-danger mr-2">Sil</a>
															<?php } }?>

														<!-- <a href="<?= base_url("senet/senet-detaylari/$senetler->senet_id"); ?>"  class="btn btn-sm btn-white text-info mr-2" target="_blank"><i class="far fa-eye mr-1"></i> Görüntüle</a> -->
...
<br><br>
			...
			<div id="edit_category" class="modal custom-modal fade" role="dialog">
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title">Senet Ödemesi</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
						<div class="modal-body">
							<form action="<?= base_url("senet/senetOdeme"); ?>" method="POST">
								<input type="hidden" id="senetID" name="senetID">
								<div class="form-group">
									<label>Kasa</label>
									<select class="form-control " name="senetKasa" id="odemeKasa">
								...
								<div class="form-group">
									<label>Banka</label>
									<select class="form-control " name="senetBanka" id="odemeBanka">
								...
								<div class="form-group">
									...
									<input class="form-control" type="text" id="senetTutar" name="senetTutar" required="" readonly>
								</div>
								<div class="submit-section">
	...
	<div id="sil" class="modal custom-modal fade" role="dialog">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Senet Silme</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<form action="<?= base_url("senet/senet_sil"); ?>" method="POST">
						<input type="hidden" name="senet_id" id="senet_id" value="">
						<p id="senet_no"></p>
						<div class="submit-section">
	...
	<?php if ($this->session->flashdata('senet_sil_ok')) : ?>
		<script>
			swal({
				title: "Bilgilendirme",
				type: "success",
				text: "Senet başarıyla silindi.",
	...
	<?php endif; ?>
<?php if ($this->session->flashdata('senet_ok')): ?>
	<script>
		swal({
		  title: "Bilgilendirme",
		  type: "success", 
		  text: "Senet başarılı bir şekilde eklendi.",
		  confirmButtonText:'Tamam',
		  button: false,
		  timer: 5000,
		});
...
<?php if ($this->session->flashdata('senet_odeme_ok')): ?>
	<script>
		swal({
			title: "Bilgilendirme",
			type: "success",
			text: "Çek başarılı bir şekilde ödendi.",
			confirmButtonText:'Tamam',
			button: false,
			timer: 5000,
		});
...
<?php endif; ?>
		...
		<?php if ($this->session->flashdata('senet_odeme_sec')): ?>
			<script>
				swal({
					title: "Bilgilendirme",
					type: "error",
...
<script src="<?= base_url(); ?>assets/plugins/select2/js/select2.min.js"></script>
		...
		<script>
			$('#edit_category').on('show.bs.modal', function (event) {
				...
				$(this).find('.modal-body #senetID').val(id)
				$(this).find('.modal-body #senetTutar').val(tutar)
			})
		</script>
