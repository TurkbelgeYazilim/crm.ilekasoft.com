						...
						<div class="row">
							<div class="col-sm-10">
								...
								<ul class="breadcrumb">
									...
									<li class="breadcrumb-item active">Banka İşlemi</li>
								</ul>
							</div>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-body">
									...
									<form action="<?= base_url("banka/bankaIslemiGuncelle"); ?>" method="POST">
										<input type="hidden" name="banka_id" id="banka_id" value="<?= $bislem->bh_bankaID;?>">
										<input type="hidden" name="bh_id" id="bh_id" value="<?= $bislem->bh_id;?>">
										<div class="row">
											<div class="col-md-4">
												<div class="form-group">
													<label>Belge No</label>
													<input type="text" class="form-control" name="islem_belgeNo" required="" value="<?= $bislem->bh_belgeNumarasi;?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>Banka Adı</label>
													<input type="text" class="form-control" name="islem_bankaAdi" required="" id="ad_input" value="<?= $bankaExe->banka_bankaAd; ?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>Hesap No</label>
													<input type="text" class="form-control" name="islem_hesapNo" required="" id="no_input" value="<?= $bankaExe->banka_hesapNo; ?>">
												</div>
											</div>
											
										</div>
										<?php 
											$tarih = $bislem->bh_tarih;
										...
										<div class="row">
											<div class="col-md-4">
												<div class="form-group">
													<label>Tarih</label>
													<input type="text" class="datepicker form-control" name="islem_tarih" required="" autocomplete="off" value="<?= $yeni_tarih; ?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>Tutar</label>
													<input type="number" step="0.01" class="form-control" name="islem_tutar" required="" value="<?php if(!empty($bislem->bh_giris)){echo $bislem->bh_giris;} else if(!empty($bislem->bh_cikis)){echo $bislem->bh_cikis;} ?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>İşlem Tipi</label>
													<select name="islem_islemTipi" class="select form-control" required="">
														<option value="">İşlem tipi seçiniz...</option>
														<option value="1" <?php if(!empty($bislem->bh_giris)){echo "selected";} ?>>Giriş İşlemi</option>
														<option value="2" <?php if(!empty($bislem->bh_giris)){echo "selected";} ?>>Çıkış İşlemi</option>
													</select>
												</div>
											</div>
											<div class="col-md-12">
												<div class="form-group">
													...
													<textarea class="form-control" name="islem_aciklama"><?= $bislem->bh_aciklama;?></textarea>
												</div>
											</div>
										</div>
...
<?php if ($this->session->flashdata('banka_islemi_ok')): ?>
<script>
swal({
  title: "Bilgilendirme",
  type: "success", 
  text: "Banka işlemi başarılı bir şekilde eklendi.",
...
<?php if ($this->session->flashdata('banka_islem_updt_ok')): ?>
<script>
swal({
  title: "Bilgilendirme",
  type: "success", 
  text: "Banka işlemi başarılı bir şekilde güncellendi.",
  confirmButtonText:'Tamam',
  button: false,
  timer: 5000,
});
