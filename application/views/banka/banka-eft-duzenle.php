						...
						<div class="row">
							<div class="col-sm-10">
								...
								<ul class="breadcrumb">
									...
									<li class="breadcrumb-item active">Banka EFT Düzenle</li>
								</ul>
							</div>
		...
		<a class="btn btn-outline-light" href="javascript:history.back()"><i class="fa fa-history"></i> <br>Önceki Sayfa</a>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="card">
								<div class="card-body">
									<h4 class="card-title">EFT Bilgileri</h4>
									<form action="<?= base_url("banka/bankaEFTGuncelle"); ?>" method="POST">
										<input type="hidden" name="cari_id" id="cari_id" value="<?= $eft->bh_cariID; ?>">
										<!-- <input type="hidden" name="banka_id" id="banka_id" value="<?= $eft->bh_bankaID; ?>"> -->
										<input type="hidden" name="bh_id" id="bh_id" value="<?= $eft->bh_id; ?>">
										<input type="hidden" name="ch_id" id="ch_id" value="<?= $eft->bh_cariHareketiID; ?>">
										<div class="row">
											<div class="col-md-4">
												<div class="form-group">
													<label>Belge / EFT No</label>
													<input type="text" class="form-control" name="eft_belgeNo" required="" value="<?= $eft->bh_belgeNumarasi; ?>">
												</div>
											</div>
											<?php
												...
												<div class="form-group">
													...
													<select class="form-control select" name="banka_id">
														...
														<?php foreach($banks as $ba){?>
															<option value="<?= $ba->banka_id; ?>" <?php if($eft->bh_bankaID == $ba->banka_id){echo "selected";} ?>><?= $ba->banka_bankaAd; ?></option>
														<?php }?>
													</select>
												</div>
											</div>
											<!--<div class="col-md-4">
												<div class="form-group">
													...
													<input type="text" class="form-control" name="eft_bankaAdi" required="" id="ad_input" value="<?= $bankaExe->banka_bankaAd; ?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>Hesap No</label>
													<input type="text" class="form-control" name="eft_hesapNo" required="" id="no_input" value="<?= $bankaExe->banka_hesapNo; ?>">
												</div>
											</div> -->
											...
											<div class="col-md-6">
												<div class="form-group">
													...
													<input type="text" class="form-control" name="eft_cariAdi" required="" id="cariadi_input" value="<?= $cariExe->cari_ad; ?>">
												</div>
											</div>
											<div class="col-md-6">
												<div class="form-group">
													<label>Cari Kodu</label>
													<input type="text" class="form-control" name="eft_cariKodu" required="" id="carikodu_input" value="<?= $cariExe->cari_kodu; ?>">
												</div>
											</div>
										...
										<div class="row">
											<div class="col-md-4">
												<div class="form-group">
													<label>Tarih</label>
													<input type="text" class="datepicker form-control" name="eft_tarih" required="" value="<?= $yeni_tarih; ?>" autocomplete="off">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>Tutar</label>
													<input type="number" step="0.01" class="form-control" name="eft_tutar" required="" value="<?php if(!empty($eft->bh_giris)){echo $eft->bh_giris;} else if(!empty($eft->bh_cikis)){echo $eft->bh_cikis;} ?>">
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>İşlem Tipi</label>
													<select name="eft_islemTipi" class="select form-control" required="">
														<option value="">İşlem tipi seçiniz...</option>
														<option value="1" <?php if(!empty($eft->bh_giris)){echo "selected";} ?>>Gelen EFT</option>
														<option value="2" <?php if(!empty($eft->bh_cikis)){echo "selected";} ?>>Giden EFT</option>
													</select>
												</div>
											</div>
											<div class="col-md-12">
												<div class="form-group">
													...
													<textarea class="form-control" name="eft_aciklama"><?= $eft->bh_aciklama;?></textarea>
												</div>
											</div>
										</div>
...
<?php if ($this->session->flashdata('banka_eft_ok')): ?>
<script>
swal({
  title: "Bilgilendirme",
  type: "success", 
  text: "Banka EFT işlemi başarılı bir şekilde oluşturuldu.",
...
<?php if ($this->session->flashdata('banka_eft_updt_ok')): ?>
<script>
swal({
  title: "Bilgilendirme",
  type: "success", 
  text: "Banka EFT işlemi başarılı bir şekilde güncellendi.",
  confirmButtonText:'Tamam',
  button: false,
  timer: 5000,
});
