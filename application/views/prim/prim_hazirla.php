...
<html lang="tr">
<head>
    <title>Prim Hazırla | İlekaSoft CRM</title>
	...
	<div class="page-wrapper">
		<div class="content container-fluid">
			...
			<div class="page-header">
				<div class="row">
					<div class="col-sm-10">
						<h3 class="page-title">Prim</h3>
						<ul class="breadcrumb">
							<li class="breadcrumb-item"><a href="<?= base_url(); ?>">Anasayfa</a></li>
							<li class="breadcrumb-item">Prim</li>
							<li class="breadcrumb-item active">Prim Hazırla</li>
						</ul>
					</div>
...
<!-- Prim Seçim Modal -->
<div class="modal fade" id="primSecimModal" tabindex="-1" role="dialog" aria-labelledby="primSecimModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="primSecimModalLabel">Prim İşlemleri</h5>
      ...
      <div class="modal-body">
        <div class="text-center">
          ...
          <div class="row">
            <div class="col-6">
              <button type="button" class="btn btn-primary btn-lg btn-block prim-olustur-btn">
                <i class="fa fa-plus"></i><br>
                Prim Oluştur
              </button>
            </div>
            <div class="col-6">
              <button type="button" class="btn btn-info btn-lg btn-block primleri-gor-btn">
...
<!-- Primleri Gör Modal -->
<div class="modal fade" id="primlerGorModal" tabindex="-1" role="dialog" aria-labelledby="primlerGorModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="primlerGorModalLabel">Müşteri Primleri</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div id="primlerListesi">
          <!-- Primler buraya yüklenecek -->
          <div class="text-center">
...
$(document).ready(function() {
    ...
    $('#ilce').on('change', function() {
        ...
        if(ilce_id) {
            ...
            $.ajax({
                url: '<?= base_url("prim/get_potansiyel_mahalleler") ?>',
                type: 'POST',
    ...
    $('#potansiyel_cari_firmaTelefon').select2({
        ...
        ajax: {
            url: '<?= base_url("prim/telefon_search") ?>',
            type: 'POST',
            dataType: 'json',
            delay: 250,
    ...
    $('#potansiyelCariFiltreForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= base_url("prim/potansiyel-cari-filtrele") ?>', // Controller'da bu endpoint olmalı
            type: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            success: function(result) {
                if(result && result.status === 'success' && result.data.length > 0) {
                    ...
                    var rows = [];                    $.each(result.data, function(i, item) {
                        rows.push([
                            ...
                            '<button class="btn btn-sm btn-outline-primary potansiyel-cari-prim" data-id="'+(item.id||'')+'" data-ad="'+(item.potansiyel_cari_ad||'')+'" data-tel="'+(item.potansiyel_cari_firmaTelefon||'')+'"><i class="fa fa-file-text"></i> P</button>'
                        ]);
                    });
    ...
    $(document).on('click', '.potansiyel-cari-duzenle', function() {
        ...
        if(yeniAd !== null && yeniAd !== eskiAd) {
            $.post('prim/potansiyel-cari-guncelle', {id: id, potansiyel_cari_ad: yeniAd}, function(resp) {
    ...
    });    // Prim hazırlama işlemi - P butonuna tıklayınca seçim modalı aç
    $(document).on('click', '.potansiyel-cari-prim', function() {
        var id = $(this).data('id');
        var ad = $(this).data('ad');
        var tel = $(this).data('tel');
        
        // Seçili müşteri bilgilerini sakla
        $('#primSecimModal').data('cari-id', id);
        $('#primSecimModal').data('cari-ad', ad);
        $('#primSecimModal').data('cari-tel', tel);
        
        // Modal başlığını güncelle
        $('#seciliMusteriAdi').text('Müşteri: ' + ad);
        
        // Seçim modalını aç
        $('#primSecimModal').modal('show');
    });

    // Prim Oluştur butonuna tıklayınca
    $(document).on('click', '.prim-olustur-btn', function() {
        var modal = $('#primSecimModal');
        var id = modal.data('cari-id');
        var ad = modal.data('cari-ad');
        var tel = modal.data('cari-tel');
        
        // Seçim modalını kapat
        $('#primSecimModal').modal('hide');
        
        // Biraz bekle ve prim oluştur modalını aç
        setTimeout(function() {
    ...
    // Primleri Gör butonuna tıklayınca
    $(document).on('click', '.primleri-gor-btn', function() {
        var modal = $('#primSecimModal');
        var id = modal.data('cari-id');
        var ad = modal.data('cari-ad');
        ...
        // Biraz bekle ve primler modalını aç
        setTimeout(function() {
            // Modal başlığını güncelle
            $('#primlerGorModalLabel').text(ad + ' - Primler');
            
            // Modal aç ve primleri yükle
            $('#primlerGorModal').modal('show');
            
            // Primleri AJAX ile yükle
            $('#primlerListesi').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> Yükleniyor...</div>');
            
            $.ajax({
                url: '<?= base_url("prim/get_cari_primler") ?>',
                type: 'POST',
                data: {cari_id: id},
                dataType: 'json',
                success: function(result) {
                    if(result && result.status === 'success' && result.data.length > 0) {
                        ...
                        $('#primlerListesi').html(html);
                    } else {
                        $('#primlerListesi').html('<div class="text-center text-muted">Bu müşteri için henüz prim bulunamadı.</div>');
                    }
                },
                error: function() {
                    $('#primlerListesi').html('<div class="text-center text-danger">Primler yüklenirken hata oluştu.</div>');
                }
            });
        }, 300);
    });
    ...
    });    // Primler modal kapandığında içeriği temizle
    $('#primlerGorModal').on('hidden.bs.modal', function() {
        $('#primlerListesi').html('');
...
<script>
	// Prim hazırlama sayfası için JavaScript kodları buraya eklenebilir
</script>
</body>
</html>