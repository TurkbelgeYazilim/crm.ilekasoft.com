        ...
        <div class="modal-body">
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group">
                <label>Görsel <small class="text-muted">(İsteğe bağlı - JPG, PNG, GIF - Max: 10MB her dosya)</small></label>
                <input type="file" class="form-control" name="potansiyel_gorsel[]" id="catkapi_gorsel" accept=".jpg,.jpeg,.png,.gif" multiple>
                <small class="text-muted">İşletme görsellerini yükleyebilirsiniz (Çoklu seçim yapabilirsiniz)</small>
              </div>
...
$(document).ready(function() {    $('#il').on('change', function() {
    ...
    $('#potansiyel_cari_firmaTelefon').select2({
        ...
        minimumInputLength: 2,        ajax: {
            url: '<?= base_url("teklif/telefon-cari-search") ?>',
            type: 'POST',
            dataType: 'json',
            delay: 250,
...
<script>
	...
	$(document).ready(function() {
		...
		// Dosya yükleme validasyonu - Görsel (Çoklu)
		$('#catkapi_gorsel').on('change', function() {
			validateCatkapiMultipleImageUpload(this, 'görsel', 10);
		});
		...
		// Çoklu görsel validasyonu
		function validateCatkapiMultipleImageUpload(input, fileType, maxSizeMB) {
			...
			for (var i = 0; i < files.length; i++) {
				...
				// Dosya tipi kontrolü
				if (!allowedTypes.includes(file.type)) {
					showCatkapiFileError($container, file.name + ' dosyası için sadece JPG, PNG ve GIF dosyaları kabul edilir.');
					input.value = '';
					return false;
				}
			}
		...
		$("#catkapiForm").on('submit', function(e) {
			...
			if (gorselFile && gorselFile.files.length > 0) {
				if (!validateCatkapiMultipleImageUpload(gorselFile, 'görsel', 10)) {
					hasError = true;
				}
			}
